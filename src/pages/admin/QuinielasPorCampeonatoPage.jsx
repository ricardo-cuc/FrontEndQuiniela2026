...
import { ArrowLeft, Trophy, Calendar, Plus, Users, TrendingUp } from 'lucide-react';
...
const QuinielasPorCampeonatoPage = () => {
  ...
  const cargarDatos = async () => {
    try {
      // Cargar campeonatos para obtener el nombre
      ...
      // Cargar quinielas del campeonato
          ...
          <div key={quiniela.ID_QUINIELA} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition">
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className={`text-xs px-2 py-1 rounded-full ${getEstadoBadge(quiniela.ESTADO)}`}>
                  {quiniela.ESTADO}
                </span>
              </div>
              ...
              <div className="flex gap-2 pt-3 border-t">
                <Link
                  ...
                  className="flex-1 text-center text-sm text-indigo-600 hover:text-indigo-800"
                >
                  Estadísticas
                </Link>
                <Link
                  to={`/admin/quinielas/${quiniela.ID_QUINIELA}/ranking`}
                  className="flex-1 text-center text-sm text-green-600 hover:text-green-800"
                >
